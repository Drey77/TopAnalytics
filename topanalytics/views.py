...
class PixelView(generic.TemplateView):
	...
	def get(self, request, pixel):
		...
		print('The user\'s topuser_id is : {}, and his nickname is : {}, and his profile picture URL is : {}, and his gender is : {}'
			.format(topuser_id, nickname, profile_picture, gender))
		#les {} servent à être remplacé par ce qui se trouve entre les parenthèses après .format, dans l'ordre.
		entry = Meta_TopChretien.objects.filter(topuser_id=topuser_id)
		...
		if entry.exists():
			...
			new_user.save()
			print('Saving in database... checked topuser_id, here it is : ' + str(entry.first()))
